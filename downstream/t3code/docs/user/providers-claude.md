...
Yes, after the current turn finishes and no approval or question is waiting.

Claude providers with separate config directories still use isolated native sessions. When you
switch, T3 Code keeps the thread and workspace, starts the target Claude session fresh, and sends
bounded visible conversation history with the next request. Provider-only hidden reasoning and tool
history do not transfer.

...
Those two providers are compatible for continuation, so switching resumes the same native Codex
conversation.

A provider with a different `CODEX_HOME path` starts a fresh native session instead. T3 Code keeps
the thread and workspace, and sends bounded visible conversation history with the next request.
Provider-only hidden reasoning and tool history do not transfer.
...
That means separate native sessions. Switching still works through T3's visible-history handoff, but
most dual-account users should use the shared-home plus shadow-home setup for native continuation.

 ...
 *
 * Each environment reads provider-owned history rather than relying on T3
 * Code's orchestration projections, so usage stays complete even for turns
 * that were never driven through T3 Code. Claude and Codex use local
 * transcripts, OpenCode uses its history API, and Cursor uses its account
 * usage API.
 *
...
export const UsageBucket = Schema.Struct({
  ...
  /** Opaque source identity when one provider can expose multiple stores. */
  sourceId: Schema.String.pipe(Schema.withDecodingDefault(Effect.succeed(""))),
...
export const UsageSourceFingerprint = Schema.Struct({
  ...
  provider: UsageProviderKind,
  scope: Schema.Literals(["localFilesystem", "remoteServer", "remoteAccount"]).pipe(
    Schema.withDecodingDefault(Effect.succeed("localFilesystem" as const)),
  ),
  /** One-way source identity used to collapse remote servers and accounts. */
  sourceId: Schema.String.pipe(Schema.withDecodingDefault(Effect.succeed(""))),

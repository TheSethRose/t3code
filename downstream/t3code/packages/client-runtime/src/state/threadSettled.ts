...
/** A provider may change only at a boundary with no live or queued provider work. */
export function canSwitchProviderBetweenTurns(
  shell: Pick<
    OrchestrationThreadShell,
    "hasPendingApprovals" | "hasPendingUserInput" | "session" | "latestUserMessageAt" | "latestTurn"
  >,
  ...
  if (!canSettle(shell, options)) return false;
  if (shell.session?.activeTurnId != null) return false;

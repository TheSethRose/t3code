...
describe("canSwitchProviderBetweenTurns", () => {
  it("allows settled boundaries and blocks live sessions or callbacks", () => {
    expect(canSwitchProviderBetweenTurns(makeShell({ activityAt: FRESH }), { now: NOW })).toBe(
      true,
    );
    expect(
      canSwitchProviderBetweenTurns(makeShell({ activityAt: FRESH, sessionStatus: "running" }), {
        now: NOW,
      }),
    ).toBe(false);
    expect(
      canSwitchProviderBetweenTurns(makeShell({ activityAt: FRESH, pending: "approval" }), {
        now: NOW,
      }),
    ).toBe(false);
  });

  it("blocks active turn ids, running turns, and queued turn starts", () => {
    ...
    expect(canSwitchProviderBetweenTurns({ ...base, session: readySession }, { now: NOW })).toBe(
      false,
    );
    expect(
      canSwitchProviderBetweenTurns(
        {
          ...base,
          latestTurn: base.latestTurn ? { ...base.latestTurn, state: "running" } : null,
        },
        { now: NOW },
      ),
    ).toBe(false);

...
Invoke `$merge-t3code-downstream` for the complete update: preserve unrelated work, fetch and merge the exact `upstream/main` tip, reconcile every overlay, validate, build the DMG, fast-forward local `main`, and restore the starting worktree. The skill runs `downstream.ts roll`, `init`, `verify`, and `build` at the correct boundaries; use those commands directly only when repairing or diagnosing the workflow.

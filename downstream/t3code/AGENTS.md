...
Run `vp node downstream/tools/downstream.ts roll` from a clean `main` to fetch and merge the exact `upstream/main` tip through `sync/upstream-<sha>`. After the upstream merge, use `$merge-t3code-downstream` to reconcile every full-file overlay before running `vp node downstream/tools/downstream.ts init`; init installs repo-owned skills and copies overlays, so it must not be used as a conflict resolver.

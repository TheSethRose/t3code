...
Invoke `$t3-sync` for the complete source update: preserve unrelated work, fetch and merge the exact `upstream/main` tip, reconcile every overlay, validate once, fast-forward local `main`, and restore the starting worktree. It does not build a DMG. The skill runs `downstream.ts roll`, `inspect`, `init`, and `verify` at the correct boundaries; use those commands directly only when repairing or diagnosing the workflow.

...
export function deriveLockedProvider(input: {
  ...
  /**
   * When the thread has no live provider work (turn settled, no pending
   * callbacks), the picker is unlocked so the user can switch providers at
   * the next turn boundary. Absent/undefined keeps the historical lock.
   */
  threadIdle?: boolean;
}): ProviderDriverKind | null {
  if (!threadHasStarted(input.thread) || input.threadIdle === true) {
    return null;
  }
...
export function getStartedThreadModelChangeBlockReason(input: {
  ...
  /** When true, provider changes are allowed at the next turn boundary. */
  threadIdle?: boolean;
  currentModelSelection: ModelSelection;

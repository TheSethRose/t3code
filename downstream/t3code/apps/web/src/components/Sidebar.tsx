...
import {
  ...
  shouldStageArchiveConfirmation,
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  const showArchiveButton = variant === "card" && !isThreadRunning;
  const handleArchiveClick = useCallback(
    (event: ReactMouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (shouldStageArchiveConfirmation(confirmThreadArchive, isConfirmingArchive)) {
        setIsConfirmingArchive(true);
        requestAnimationFrame(() => archiveConfirmButtonRef.current?.focus());
        return;
      }
      setIsConfirmingArchive(false);
      onArchive(threadRef);
    },
    [confirmThreadArchive, isConfirmingArchive, onArchive, threadRef],
  );
  const handleArchiveBlur = useCallback(() => {
    requestAnimationFrame(() => {
      if (!archiveConfirmButtonRef.current?.contains(document.activeElement)) {
        setIsConfirmingArchive(false);
      }
    });
  }, []);
          ...
          <div className="relative z-10 h-[4.875rem] px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]">
            <div className="flex h-5 min-w-0 items-center gap-1.5">
              ...
              <span className="group/sidebar-status-slot relative ml-auto flex h-5 min-w-8 shrink-0 items-stretch justify-end text-xs">
                ...
                {showArchiveButton || props.settlementSupported || showSnoozeButton ? (
                  <span
                    className={cn(
                    ...
                    {showArchiveButton ? (
                      isConfirmingArchive ? (
                        <button
                          ...
                          onBlur={handleArchiveBlur}
                          className="inline-flex cursor-pointer items-center gap-1 rounded-md bg-transparent px-1.5 text-xs text-destructive hover:text-destructive/80"
                        >
                          Confirm
                        </button>
                      ) : (

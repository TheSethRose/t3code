...
describe("mergeUsage", () => {
  ...
  it("collapses an account-wide provider source across environments", () => {
    const cursorBucket = bucket({ provider: "cursor", model: "gpt-5", sourceId: "account-a" });
    const cursorSource = {
      provider: "cursor" as const,
      hostId: "ignored",
      homePath: "Cursor account",
      scope: "remoteAccount" as const,
      sourceId: "account-a",
    };
    const makeRemoteSummary = (): UsageSummary => {
      const value = summary([cursorBucket], []);
      return {
        ...value,
        sources: [
          {
            fingerprint: {
              hostId: cursorSource.hostId,
              provider: cursorSource.provider,
              scope: cursorSource.scope,
              sourceId: cursorSource.sourceId,
              resolvedHomePath: cursorSource.homePath,
              volumeId: "",
            },
            status: "ok",
            scannedFiles: 0,
    ...
    const merged = mergeUsage(
      [environment("env-a", makeRemoteSummary()), environment("env-b", makeRemoteSummary())],
      USAGE_CONTRACT_VERSION,
    );
    ...
    const failed = makeRemoteSummary();
    const recovered = mergeUsage(
      [
        environment("env-a", {
          ...failed,
          buckets: [],
          sources: failed.sources.map((source) => ({ ...source, status: "failed" as const })),
        }),
        environment("env-b", makeRemoteSummary()),
      ],
      USAGE_CONTRACT_VERSION,
    );
    expect(recovered.costUsd).toBe(10);
    expect(recovered.contributingEnvironments).toEqual(["env-b"]);
    expect(recovered.sourceWarnings).toEqual(["env-a: cursor usage is incomplete."]);
  });

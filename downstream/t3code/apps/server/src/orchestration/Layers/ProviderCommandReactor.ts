...
// Provider handoff: a bounded, provider-only rendering of prior visible T3
// messages prepended to the target turn. It is passed as turn input only and
// never persisted as a T3 message, activity, or log entry.
const PROVIDER_HANDOFF_HEADER =
  "Continue this conversation. Prior conversation history (from T3, not this provider):\n\n";
const PROVIDER_HANDOFF_TRUNCATION_MARKER = "[Earlier conversation history truncated]\n\n";
const decodeProviderSegmentRuntimePayload = Schema.decodeUnknownOption(
  Schema.Struct({ segmentStartTurnCount: Schema.optional(NonNegativeInt) }),
);
...
/**
 * Builds the deterministic provider-only history handed to a fresh session
 * when a thread changes provider identity (or rehydrates after a revert below
 * the current native segment). Chronological role order is preserved; oldest
 * content drops first under the send-turn input budget with a fixed marker.
 * The triggering user message is excluded so it is sent exactly once as the
 * active request.
 */
export function renderProviderHandoffHistory(
  messages: ReadonlyArray<ThreadTitleMessage & { readonly id: MessageId }>,
  ...
  const sections = messages.flatMap((message) => {
    ...
    const text = formatThreadTitleSection(message);
    return text === undefined
      ? []
      : [{ text, attachments: message.attachments ?? [], chars: text.length }];
  });
  if (sections.length === 0) return null;

  const separator = "\n\n";
  const activeInputSeparatorChars = reserveChars > 0 ? separator.length : 0;
  const historyBudget = Math.max(
    0,
    PROVIDER_SEND_TURN_MAX_INPUT_CHARS -
      PROVIDER_HANDOFF_HEADER.length -
      activeInputSeparatorChars -
      reserveChars,
  );
  const fullHistory = sections.map((section) => section.text).join(separator);
  let retained = sections;
  const truncated = fullHistory.length > historyBudget;

  if (truncated) {
    const contentBudget = historyBudget - PROVIDER_HANDOFF_TRUNCATION_MARKER.length;
    if (contentBudget <= 0) return null;
    retained = [];
    let used = 0;
    for (const section of sections.toReversed()) {
      const separatorChars = retained.length > 0 ? separator.length : 0;
      const available = contentBudget - used - separatorChars;
      if (available <= 0) break;
      if (section.chars > available) {
        if (retained.length === 0) {
          const text = section.text.slice(-available);
          retained.unshift({ ...section, text, chars: text.length });
        }
        break;
      }
      retained.unshift(section);
      used += separatorChars + section.chars;
    }
    if (retained.length === 0) return null;
  }

  const text = `${PROVIDER_HANDOFF_HEADER}${truncated ? PROVIDER_HANDOFF_TRUNCATION_MARKER : ""}${retained
    .map((section) => section.text)
    .join(separator)}`;
  const attachments = retained
    .toReversed()
...
const make = Effect.gen(function* () {
  ...
  const threadHasLiveProviderWork = (thread: OrchestrationThreadShell): boolean =>
  ...
  const ensureSessionForThread = Effect.fn("ensureSessionForThread")(function* (
    ...
    const hasCompatiblePersistedCursor =
      persistedBinding?.providerInstanceId === desiredInstanceId &&
      persistedBinding.resumeCursor !== null &&
      persistedBinding.resumeCursor !== undefined;
    const persistedSegmentStartTurnCount = Option.getOrUndefined(
      decodeProviderSegmentRuntimePayload(persistedBinding?.runtimePayload),
    )?.segmentStartTurnCount;
    ...
    const threadHadLiveProviderWork = threadHasLiveProviderWork(threadShell);
    ...
    // Provider handoff: a request to a different provider instance whose
    // driver or continuation identity differs from the bound session cannot
    // reuse native continuation state. The old session is stopped, a fresh
    // session starts without a cursor, and the caller prepends a bounded
    // rendering of prior visible T3 history to the turn. Compatible
    // instances (same driver, same continuation key) keep the existing
    // resume-with-cursor restart path below.
    let forceFreshHandoff = false;
    if (requestedInstanceChanged) {
      if (threadHadLiveProviderWork) {
        return yield* new ProviderAdapterRequestError({
          provider: preferredProvider,
          method: "thread.turn.start",
          detail: `Thread '${threadId}' cannot switch providers while a turn or provider callback is live. Wait until the thread settles.`,
        });
      }
      forceFreshHandoff =
    ...
    const currentCheckpointTurnCount = thread.checkpoints.reduce(
    ...
    const startProviderSession = (input?: {
      ...
      readonly segmentStartTurnCount?: number;
    }) =>
      providerService.startSession(threadId, {
        ...
        ...(input?.segmentStartTurnCount !== undefined
          ? { segmentStartTurnCount: input.segmentStartTurnCount }
          : {}),
        runtimeMode: desiredRuntimeMode,
      });
    ...
    if (existingSessionThreadId) {
      ...
      if (
        ...
        return {
          ...
          needsHistoryHandoff: false,
        };
      }

      const resumeCursor = forceFreshHandoff
        ? undefined
        : shouldRestartForModelChange
          ? undefined
      ...
      yield* Effect.logInfo("provider command reactor restarting provider session", {
        ...
        cwdChanged,
        modelChanged,
        instanceChanged: requestedInstanceChanged,
        shouldRestartForModelChange,
        shouldRestartForModelSelectionChange,
        forceFreshHandoff,
      ...
      const willResume =
        resumeCursor !== undefined || (!forceFreshHandoff && hasCompatiblePersistedCursor);
      const restartedSession = yield* startProviderSession(
        resumeCursor !== undefined
          ? {
              resumeCursor,
              ...(persistedSegmentStartTurnCount !== undefined
                ? { segmentStartTurnCount: persistedSegmentStartTurnCount }
                : {}),
            }
          : !willResume
            ? { segmentStartTurnCount: currentCheckpointTurnCount }
            : undefined,
      );
      ...
      return {
        ...
        needsHistoryHandoff: !willResume,
      };
    }
    ...
    return {
      ...
      needsHistoryHandoff: !hasCompatiblePersistedCursor,
    };
  });
  ...
  const buildSendTurnRequestForThread = Effect.fnUntraced(function* (input: {
    ...
    // Provider handoff: when this turn targets a session started without
    // native continuation (incompatible instance switch, or a revert below
    // the current native segment) and the thread already has visible
    // history, prepend a bounded provider-only rendering of the prior T3
    // messages. The triggering message is excluded so it is sent exactly
    // once as the active request; the synthetic text is never persisted as
    // a T3 message or activity.
    let combinedInput = normalizedInput;
    let combinedAttachments = normalizedAttachments;
    if (ensuredSession.needsHistoryHandoff && thread.messages.length > 0) {
      const handoff = renderProviderHandoffHistory(
        thread.messages.toReversed(),
        input.excludeMessageId,
        normalizedInput?.length ?? 0,
      );
      if (handoff !== null) {
        combinedInput = `${handoff.text}${
          normalizedInput !== undefined ? `\n\n${normalizedInput}` : ""
        }`;
        combinedAttachments = [...normalizedAttachments, ...handoff.attachments].slice(
          0,
          PROVIDER_SEND_TURN_MAX_ATTACHMENTS,
        );
      }
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    const handleTurnStartFailure = (cause: Cause.Cause<unknown>) => {
      ...
      const preservesLiveSession =
        isProviderAdapterRequestError(failure) &&
        failure.method === "thread.turn.start" &&
        failure.detail.includes(
          "cannot switch providers while a turn or provider callback is live",
        );
      return (

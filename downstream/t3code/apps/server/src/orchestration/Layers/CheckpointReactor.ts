...
function readSegmentStartTurnCount(runtimePayload: unknown): number | undefined {
  return Option.getOrUndefined(decodeProviderSegmentRuntimePayload(runtimePayload))
...
const make = Effect.gen(function* () {
  ...
  const handleRevertRequested = Effect.fn("handleRevertRequested")(function* (
    ...
    // A revert below the current provider segment start (a provider handoff
    // boundary) invalidates the native conversation: it extends past the
    // revert point and can no longer be trusted. Discard the binding and the
    // session so the next turn starts fresh and rehydrates from reverted T3
    // history. Within the current segment, the provider conversation rolls
    // back as usual.
    const binding = Option.getOrUndefined(
      yield* providerSessionDirectory.getBinding(event.payload.threadId),
    );
    const segmentStartTurnCount = readSegmentStartTurnCount(binding?.runtimePayload);
    const belowCurrentSegment =
      segmentStartTurnCount !== undefined && event.payload.turnCount < segmentStartTurnCount;
    if (belowCurrentSegment) {
      if (binding) {
        yield* providerService.stopSession({ threadId: binding.threadId }).pipe(
          Effect.catchCause((cause) =>
            Effect.logWarning("checkpoint reactor failed to stop discarded provider segment", {
        ...
        yield* providerSessionDirectory.upsert({
          ...
          ...(binding.providerInstanceId ? { providerInstanceId: binding.providerInstanceId } : {}),
          status: "stopped",
          resumeCursor: null,
          runtimePayload: null,
        });

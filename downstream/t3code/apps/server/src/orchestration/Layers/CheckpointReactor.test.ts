...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const providerSessionDirectory = ProviderSessionDirectory.of({
      ...
      getBinding: (threadId) =>
        Effect.succeed(
          providerBinding?.threadId === threadId ? Option.some(providerBinding) : Option.none(),
        ),
      listThreadIds: () =>
      ...
      listBindings: () =>
        Effect.succeed(
          providerBinding === undefined
            ? []
            : [{ ...providerBinding, lastSeenAt: "2026-01-01T00:00:00.000Z" }],
        ),
    });
  ...
  it("discards native binding when revert crosses the current provider segment", async () => {
    const harness = await createHarness();
    const createdAt = "2026-01-01T00:00:00.000Z";
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-segment-session-set"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
    ...
    await runtime!.runPromise(
      harness.providerSessionDirectory.upsert({
        ...
        providerInstanceId: ProviderInstanceId.make("codex"),
        status: "running",
        resumeCursor: { opaque: "segment-cursor" },
        runtimePayload: { segmentStartTurnCount: 2, providerSpecific: "stale" },
      }),
    );
    ...
    for (const turnCount of [1, 2]) {
      await runtime!.runPromise(
        harness.engine.dispatch({
          type: "thread.turn.diff.complete",
          commandId: CommandId.make(`cmd-segment-diff-${turnCount}`),
          threadId: ThreadId.make("thread-1"),
          turnId: asTurnId(`segment-turn-${turnCount}`),
          completedAt: createdAt,
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.checkpoint.revert",
        commandId: CommandId.make("cmd-segment-revert"),
        threadId: ThreadId.make("thread-1"),
        turnCount: 1,
        createdAt,
      }),

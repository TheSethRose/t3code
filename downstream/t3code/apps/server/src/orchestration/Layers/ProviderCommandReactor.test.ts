...
describe("ProviderCommandReactor", () => {
  ...
  describe("provider handoff history", () => {
    it("drops oldest text deterministically within the provider input limit", () => {
      const currentMessageId = asMessageId("current-message");
      const rendered = renderProviderHandoffHistory(
        [
          {
            id: asMessageId("old-message"),
            role: "user",
            text: `old:${"x".repeat(PROVIDER_SEND_TURN_MAX_INPUT_CHARS)}`,
            attachments: [],
          },
          {
            id: asMessageId("recent-message"),
            role: "assistant",
            text: "recent answer",
      ...
      expect(
        (rendered?.text.length ?? 0) + "\n\n".length + "current request".length,
      ).toBeLessThanOrEqual(PROVIDER_SEND_TURN_MAX_INPUT_CHARS);
      expect(rendered?.text).toContain("[Earlier conversation history truncated]");
      expect(rendered?.text).toContain("recent answer");
      expect(rendered?.text).not.toContain("current request");
      expect(rendered?.attachments.map((attachment) => attachment.id)).toEqual(
        Array.from(
          { length: PROVIDER_SEND_TURN_MAX_ATTACHMENTS },
          (_, index) => `recent-image-${index}`,
        ),
      );
    });
  });
  ...
  async function createHarness(input?: {
    ...
    let providerBinding: ProviderRuntimeBinding | undefined;
    ...
    const startSession = vi.fn((_: unknown, input: unknown) => {
      ...
      return (startSessionEffect?.(session) ?? Effect.succeed(session)).pipe(
        Effect.tap((startedSession) =>
          Effect.sync(() => {
            const previousSessionIndex = runtimeSessions.findIndex(
              (existing) => existing.threadId === startedSession.threadId,
            );
            if (previousSessionIndex >= 0) runtimeSessions.splice(previousSessionIndex, 1);
            runtimeSessions.push(startedSession);
            const segmentStartTurnCount =
              typeof input === "object" &&
              input !== null &&
              "segmentStartTurnCount" in input &&
              typeof input.segmentStartTurnCount === "number"
                ? input.segmentStartTurnCount
                : undefined;
            providerBinding = {
              ...
              runtimePayload:
                segmentStartTurnCount === undefined ? null : { segmentStartTurnCount },
            };
          }),
        ),
      );
    ...
    const providerSessionDirectory = ProviderSessionDirectory.of({
      ...
      getProvider: () =>
        providerBinding
          ? Effect.succeed(providerBinding.provider)
          : Effect.die(new Error("Provider binding was not initialized.")),
      getBinding: () =>
        Effect.succeed(
          providerBinding === undefined ? Option.none() : Option.some(providerBinding),
        ),
      listThreadIds: () =>
        Effect.succeed(providerBinding === undefined ? [] : [providerBinding.threadId]),
      listBindings: () =>
        Effect.succeed(
          providerBinding === undefined ? [] : [{ ...providerBinding, lastSeenAt: now }],
        ),
    });
  ...
  it("restarts an existing Codex thread on a compatible requested instance", async () => {
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-compatible-codex-ready"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
    ...
    expect(harness.startSession.mock.calls[1]?.[1]).toMatchObject({
      ...
      segmentStartTurnCount: 0,
    });
    expect(harness.sendTurn.mock.calls[1]?.[0]).toMatchObject({ input: "second" });
  ...
  it("hands visible history exactly once across incompatible switches in both directions", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-provider-handoff-first"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("provider-handoff-first"),
          role: "user",
          text: "first context",
          attachments: [historicalImage],
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
        createdAt: now,
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-provider-handoff-codex-ready"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-provider-handoff-claude"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("provider-handoff-claude"),
          role: "user",
    ...
    const claudeStart = harness.startSession.mock.calls[1]?.[1];
    expect(claudeStart).toMatchObject({
      providerInstanceId: ProviderInstanceId.make("claudeAgent"),
      segmentStartTurnCount: 0,
    });
    expect(claudeStart).not.toHaveProperty("resumeCursor");
    const claudeTurn = harness.sendTurn.mock.calls[1]?.[0] as {
      input: string;
      attachments: ReadonlyArray<{ readonly id: string }>;
    };
    expect(claudeTurn.input.match(/first context/g)).toHaveLength(1);
    expect(claudeTurn.input.match(/switch to claude/g)).toHaveLength(1);
    expect(claudeTurn.input.indexOf("first context")).toBeLessThan(
      claudeTurn.input.indexOf("switch to claude"),
    );
    expect(claudeTurn.attachments.map((attachment) => attachment.id)).toEqual([
      "current-image",
      "historical-image",
    ]);
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-provider-handoff-claude-ready"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-provider-handoff-back-to-codex"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("provider-handoff-back-to-codex"),
          role: "user",
          text: "switch back",
          attachments: [],
        },
    ...
    const codexTurn = harness.sendTurn.mock.calls[2]?.[0] as { input: string };
    expect(codexTurn.input.match(/first context/g)).toHaveLength(1);
    expect(codexTurn.input.match(/switch to claude/g)).toHaveLength(1);
    expect(codexTurn.input.match(/switch back/g)).toHaveLength(1);
    expect(codexTurn.input.indexOf("first context")).toBeLessThan(
      codexTurn.input.indexOf("switch to claude"),
    );
    expect(codexTurn.input.indexOf("switch to claude")).toBeLessThan(
      codexTurn.input.indexOf("switch back"),
    );
    ...
    expect(
      thread?.messages.some((message) => message.text.includes("Prior conversation history")),
    ).toBe(false);
  });
  ...
  it("keeps the previous provider binding recoverable when target startup fails", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-provider-switch-failure-first"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("provider-switch-failure-first"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-provider-switch-failure-ready"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
    ...
    harness.startSession.mockImplementationOnce(
      (_threadId: unknown, _input: unknown) =>
        Effect.fail("simulated target startup failure") as never,
    );
    await runtime!.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        ...
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("provider-switch-failure-target"),
          role: "user",
          text: "switch",
          attachments: [],
        },
    ...
    expect(harness.sendTurn).toHaveBeenCalledTimes(1);
    expect(harness.providerBinding).toMatchObject({
      provider: ProviderDriverKind.make("codex"),
      providerInstanceId: ProviderInstanceId.make("codex"),
      resumeCursor: { opaque: "resume-1" },
    });
  ...
  it("rejects provider changes while the current provider turn is still live", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("starts a fresh cross-driver session after the existing thread session has stopped", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";

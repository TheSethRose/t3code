...
describe("CodexSessionRuntime collab integration", () => {
  ...
  it.live("Stop interrupts every live child regardless of registration timing", () =>
    Effect.gen(function* () {
      ...
      // Stop everything. A's interrupt hangs forever, so the shared deadline
      // must fail while B's and the parent's concurrent requests are still sent.
      const interruptExit = yield* runtime.interruptTurn().pipe(Effect.exit);
      assert.isTrue(Exit.isFailure(interruptExit), "a hung child interrupt must fail Stop");
      ...
      assert.isTrue(interruptedThreads.has(ROOT), "parent turn must be interrupted concurrently");

  ...
  it.effect("routes provider operations and rollback conversation", () =>
    Effect.gen(function* () {
      ...
      const session = yield* provider.startSession(asThreadId("thread-1"), {
        ...
        cwd: "/tmp/project",
        runtimeMode: "full-access",
        segmentStartTurnCount: 4,
      });
      assert.equal(session.provider, "codex");
      const runtimeRepository = yield* ProviderSessionRuntime.ProviderSessionRuntimeRepository;
      const persistedSession = yield* runtimeRepository.getByThreadId({
        threadId: session.threadId,
      });
      assert.equal(Option.isSome(persistedSession), true);
      if (Option.isSome(persistedSession)) {
        assert.deepInclude(persistedSession.value.runtimePayload as Record<string, unknown>, {
          cwd: "/tmp/project",
          model: null,
          segmentStartTurnCount: 4,
        });

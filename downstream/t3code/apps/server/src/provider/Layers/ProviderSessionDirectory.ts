...
function mergeRuntimePayload(
  ...
  if (next === undefined && !replace) {
    return existing ?? null;
  }
  if (!replace && isRecord(existing) && isRecord(next)) {
...
const makeProviderSessionDirectory = Effect.gen(function* () {
  ...
  const upsert: ProviderSessionDirectoryShape["upsert"] = Effect.fn(function* (binding) {
    ...
    const existingProvider = existingRuntime?.providerName;
    const existingInstanceId =
      existingRuntime?.providerInstanceId ??
      (existingProvider
        ? defaultInstanceIdForDriver(ProviderDriverKind.make(existingProvider))
        : undefined);
    const providerChanged = existingRuntime !== undefined && existingProvider !== binding.provider;
    const providerInstanceId =
      binding.providerInstanceId ??
      (providerChanged || existingInstanceId === undefined
        ? defaultInstanceIdForDriver(binding.provider)
        : existingInstanceId);
    // A provider identity change (driver or routing instance) invalidates
    // every provider-native continuation artifact. Omitted instance ids keep
    // the current same-driver identity; legacy/new bindings use that driver's
    // default instance.
    const identityChanged =
      existingRuntime !== undefined &&
      (providerChanged || existingInstanceId !== providerInstanceId);
    yield* repository
      .upsert({
        ...
        adapterKey:
          binding.adapterKey ??
          (identityChanged ? binding.provider : (existingRuntime?.adapterKey ?? binding.provider)),

...
class FakeCodexRuntime implements CodexSessionRuntimeShape {
  ...
  public readonly interruptTurnImpl = vi.fn((_turnId?: TurnId): void => undefined);

  public interruptTurnEffect: Effect.Effect<void, CodexErrors.CodexAppServerRequestError> =
    Effect.void;
  ...
  interruptTurn(turnId?: TurnId) {
    this.interruptTurnImpl(turnId);
    return this.interruptTurnEffect;
  }
...
scopedLifecycleLayer("CodexAdapterLive scoped lifecycle", (it) => {
  ...
  it.effect("keeps the Codex session when interrupt succeeds", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;

      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-graceful-stop"),
        runtimeMode: "full-access",
      });
      const runtime = scopedLifecycleRuntimeFactory.lastRuntime;
      NodeAssert.ok(runtime);

      yield* adapter.interruptTurn(asThreadId("thread-graceful-stop"), asTurnId("turn-1"));

      NodeAssert.deepStrictEqual(runtime.interruptTurnImpl.mock.calls, [[asTurnId("turn-1")]]);
      NodeAssert.equal(runtime.closeImpl.mock.calls.length, 0);
      NodeAssert.equal(yield* adapter.hasSession(asThreadId("thread-graceful-stop")), true);
    }),
  );

  it.effect("closes only the failing Codex session when interrupt is rejected", () =>
    Effect.gen(function* () {
      scopedLifecycleRuntimeFactory.releasedThreadIds.length = 0;
      const adapter = yield* CodexAdapter;
      ...
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-failing-stop"),
        runtimeMode: "full-access",
      });
      const failingRuntime = scopedLifecycleRuntimeFactory.lastRuntime;
      NodeAssert.ok(failingRuntime);

      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-still-running"),
        runtimeMode: "full-access",
      });
      const otherRuntime = scopedLifecycleRuntimeFactory.lastRuntime;
      NodeAssert.ok(otherRuntime);

      failingRuntime.interruptTurnEffect = Effect.fail(
        new CodexErrors.CodexAppServerRequestError({
          code: -32603,
          errorMessage: "expected active turn id old but found new",
        }),
      );

      yield* adapter.interruptTurn(asThreadId("thread-failing-stop"), asTurnId("old"));

      NodeAssert.equal(failingRuntime.closeImpl.mock.calls.length, 1);
      NodeAssert.equal(otherRuntime.closeImpl.mock.calls.length, 0);
      NodeAssert.equal(yield* adapter.hasSession(asThreadId("thread-failing-stop")), false);
      NodeAssert.equal(yield* adapter.hasSession(asThreadId("thread-still-running")), true);
    }),
  );

  it.effect("closes a Codex session when interrupt does not finish within five seconds", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;

      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-hung-stop"),
        runtimeMode: "full-access",
      });
      const runtime = scopedLifecycleRuntimeFactory.lastRuntime;
      NodeAssert.ok(runtime);
      ...
      const interruptFiber = yield* adapter
        .interruptTurn(asThreadId("thread-hung-stop"))
        .pipe(Effect.forkChild);
      yield* TestClock.adjust("5 seconds");
      yield* Fiber.join(interruptFiber);

      NodeAssert.equal(runtime.closeImpl.mock.calls.length, 1);
      NodeAssert.equal(yield* adapter.hasSession(asThreadId("thread-hung-stop")), false);

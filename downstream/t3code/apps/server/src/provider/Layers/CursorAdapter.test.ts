  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const promptParams = promptRequests[0]?.params as Record<string, unknown>;
        assert.deepStrictEqual(Object.keys(promptParams).sort(), ["prompt", "sessionId"]);
        assert.deepStrictEqual(promptParams.prompt, [{ type: "text", text: "hello mock" }]);
      }),
  );

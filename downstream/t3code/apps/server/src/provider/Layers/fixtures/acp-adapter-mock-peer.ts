...
const sessionId = "acp-adapter-test-session";
...
const program = Effect.gen(function* () {
  ...
  yield* agent.handleInitialize(() =>
    Effect.succeed({
      protocolVersion: 1,
      agentCapabilities: {},
      agentInfo: { name: "acp-adapter-test", version: "0.0.0" },
      authMethods: [
  ...
  yield* agent.handleCancel(() => Deferred.succeed(cancelled, undefined).pipe(Effect.asVoid));

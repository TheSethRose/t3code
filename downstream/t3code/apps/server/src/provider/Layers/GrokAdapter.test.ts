...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
  ...
  it.effect("does not let a cancelled prompt settlement consume the follow-up prompt slot", () =>
    Effect.gen(function* () {
      ...
      const promptRequests = (yield* Effect.promise(() => readJsonLines(requestLogPath))).filter(
        (entry) => entry.method === "session/prompt",
      );
      assert.equal(promptRequests.length, 2);
      for (const promptRequest of promptRequests) {
        const params = promptRequest.params as Record<string, unknown>;
        assert.deepEqual(Object.keys(params).sort(), ["_meta", "prompt", "sessionId"]);
      }
      assert.deepEqual(
        promptRequests.map(
          (entry) =>
            ((entry.params as Record<string, unknown>).prompt as Array<Record<string, unknown>>)[0]
              ?.text,
        ),
        ["cancel this prompt", "complete the follow-up"],
      );

...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))("ProviderSessionDirectoryLive", (it) => {
  ...
  it("persists runtime fields and merges payload updates", () =>
    Effect.gen(function* () {
      ...
      if (Option.isSome(runtime)) {
        ...
        assert.equal(runtime.value.providerInstanceId, ProviderInstanceId.make("codex_personal"));
        assert.equal(runtime.value.status, "running");
  ...
  it("clears foreign cursor and runtime payload when the provider instance changes", () =>
    Effect.gen(function* () {
      const directory = yield* ProviderSessionDirectory;
      const runtimeRepository = yield* ProviderSessionRuntime.ProviderSessionRuntimeRepository;
      ...
      yield* directory.upsert({
        ...
        providerInstanceId: ProviderInstanceId.make("codex_personal"),
        threadId,
        resumeCursor: { opaque: "personal-cursor" },
        runtimePayload: { cwd: "/tmp/personal", segmentStartTurnCount: 3 },
      });
      yield* directory.upsert({
        provider: ProviderDriverKind.make("codex"),
      ...
      assert.equal(Option.isSome(runtime), true);
      if (Option.isSome(runtime)) {
        assert.equal(runtime.value.providerInstanceId, ProviderInstanceId.make("codex_work"));

...
export function resolveAcpAuthMethodId(
  ...
  const configured = configuredMethodId.trim();
  if (configured) return configured;
  return initializeResult.authMethods?.find((method) => !("type" in method))?.id.trim() ?? "";
}
  ...
  Effect.gen(function* () {
    ...
    const startOnce = Effect.gen(function* () {
      ...
      const authenticatePayload = {
        methodId: resolveAcpAuthMethodId(options.authMethodId, initializeResult),

...
function fingerprintKey(fingerprint: UsageSourceFingerprint): string {
  if (fingerprint.scope !== "localFilesystem" && fingerprint.sourceId.length > 0) {
    return [fingerprint.provider, fingerprint.scope, fingerprint.sourceId].join(" ");
  }
  return [
    fingerprint.hostId,
...
function ownedContribution(
  ...
  for (const source of environment.summary.sources) {
    ...
    if (ownerByFingerprint.get(key) === environment.environmentId) {
      ...
      if (source.fingerprint.sourceId.length > 0) {
        ownedSourceIds.add(source.fingerprint.sourceId);
      }

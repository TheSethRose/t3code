  ...
  return {
    ...
    settings: {
      ...
      developer_instructions: buildCodexDeveloperInstructions(
        ...
        { hasPreviewTools: input.hasPreviewTools ?? false },
      ),
    },
  };
}
    ...
    return {
      ...
      sendTurn: (input) =>
        Effect.gen(function* () {
          ...
          const params = yield* buildTurnStartParams({
            ...
            ...(input.effort ? { effort: input.effort } : {}),
            ...(input.interactionMode ? { interactionMode: input.interactionMode } : {}),
            hasPreviewTools: hasConfiguredMcpServer(options.appServerArgs),
          });
      ...
      interruptTurn: (turnId) =>
        Effect.gen(function* () {
          ...
          const interrupts = Array.from(liveChildTurns.entries()).map(
          ...
          const interrupted = yield* Effect.all(interrupts, {
          ...
          if (Option.isNone(interrupted)) {
            return yield* new CodexErrors.CodexAppServerRequestError({
              code: -32603,
              errorMessage: "Codex turn interruption timed out after five seconds.",
